...
rsync -e "ssh -i %sshkey% -o StrictHostKeychecking=no" -avz --exclude /node-server --exclude .git --exclude .hg --exclude /specs --exclude /node_modules --exclude Gruntfile.js --exclude package.json --exclude .gitignore --exclude .hgignore --exclude build.bat --exclude sync.bat . ubuntu@%remote%:/var/www/
